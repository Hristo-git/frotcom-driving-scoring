...
export default function DashboardClient({
    ...
    const applyWithDates = (newStart: string, newEnd: string) => {
        ...
        // Persist current filters in URL so they survive the reload
    ...
    const filteredDrivers = React.useMemo(() => drivers.filter(d => {
        ...
        if (selectedCountry.length > 0 && !selectedCountry.includes(d.country)) return false;
        if (selectedWarehouse.length > 0 && !selectedWarehouse.includes(d.warehouse)) return false;
        // Brand/model filter
    ...
    }), [drivers, selectedCountry, selectedWarehouse, selectedBrand, selectedModel, vehicleMap]);
    ...
    const handleExportExcel = () => {
        const data = sortedDrivers.map(d => ({
            'ШОФЬОР': d.driverName,
            'ТОЧКИ': Number(d.score.toFixed(2)),
            'СРЕДНО ВРЕМЕ': formatTime(d.drivingTime),
            'СРЕДЕН РАЗХОД (L/100km)': d.consumption > 0 ? Number(d.consumption.toFixed(1)) : 0,
            'КИЛОМЕТРИ': Number(d.distance.toFixed(1)),
            'РАБОТА НА МЯСТО (%)': d.idling,
            'АВТОМОБИЛИ': d.vehicles.join(', ')
        }));

        const ws = XLSX.utils.json_to_sheet(data);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, 'Шофьори');
